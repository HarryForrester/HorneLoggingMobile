...
type HazardProps = {name: string; hazards: []}

const Hazard = ({name}: HazardProps) => (
  <View>
    <Text>{name}</Text>
  </View>
)
...
const JobScreen = () => {
  ...
  useEffect(() => {
    ...
    generalHazards
      ...
      .map((gh: any) => {
        let parsedGH = []
        try {
          parsedGH = JSON.parse(gh.hazards)
  ...
  if (isLoading) {
    return (
      ...
      >
        <ActivityIndicator
          size='large'
          color={isDarkMode ? '#fff' : '#0c3424'}
        />
        <Text style={isDarkMode ? styles.loadingDark : styles.loadingLight}>
          Loading...
        </Text>
      </View>
    )
  ...
  const renderMapButton = ({item}: any) => (
    <TouchableOpacity
      style={[
        styles.pdfButton,
        {backgroundColor: isDarkMode ? 'grey' : '#0c3424'},
      ]}
  ...
  return (
    <SafeAreaView style={isDarkMode ? styles.darkmode : styles.normal}>
      ...
      <View style={{marginTop: 40}}>
        <View
          style={[styles.buttonContainer, isDarkMode && styles.containerDark]}
        >
...
const PDFViewer = ({
  ...
  useEffect(() => {
    try {
      ...
      console.error('Error occurred while getting cut plan')
    }
  }, [selectedMarker])
  ...
  useEffect(() => {
    const deviceString = app.currentUser?.customData.device as
      | string
      | undefined
  ...
  async function handleHazardPress(hazardNames: any) {
    try {
      const data = hazards
      if (data) {
        const filteredHazards = hazards.filter((hazard: {id: Hazard}) =>
  ...
  async function handleGeneralHazardPress(hazardNames: any) {
    ...
    if (parsedData) {
      const filteredHazards = hazards.filter(
        (hazard: {id: Hazard}) => hazard.id === hazardNames.id,
      )
      setSelectedHazards(filteredHazards)
    }
  }
      ...
      >
            ...
            >
              <ScrollView>
                {(accessLevelAdmin === 'on' || accessLevelForeman === 'on') && (
                  <>
                    ...
                    <FlatList
                      ...
                      renderItem={({item}) => (
                        <TouchableOpacity
                          onPress={() => handleCrewMemberPress(item)}
                        >
                          <Text
                            style={[
                              styles.listItem,
                ...
                {(accessLevelAdmin === 'on' || accessLevelForeman === 'on') && (
                  <>
                    ...
                    <FlatList
                      ...
                      renderItem={({item}) => (
                        <TouchableOpacity
                          onPress={() => handleDocumentPress(item.uri)}
                        >
                          <Text
                            style={[
                              styles.listItem,
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => handleCutPlanFilePress(item.base64String)}
                    >
                      <Text
                        style={[
                          styles.listItem,
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => handleHazardPress(item.id)}
                    >
                      <Text
                        style={[
                          styles.listItem,
                ...
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => handleGeneralHazardPress(item)}
                    >
                      <Text
                        style={[
                          styles.listItem,
        ...
        >
          <View style={styles.modalContainer}>
            ...
            >
              {selectedCrewMember &&
                Object.entries(selectedCrewMember).map(
                  ([crewName, crewMembers]) => {
                    if (crewName) {
                      return (
                        <View key={crewName}>
                          <Text
            ...
            >
              <ScrollView>
                <View style={{padding: 10}}>
                  ...
                  <View style={styles.personDetailRow}>
                    <Text style={[styles.personDetailLabel, darkMode]}>
                      Role:
                    </Text>
                  ...
                  <View style={styles.personDetailRow}>
                    ...
                    <TouchableOpacity
                      onPress={() =>
                        Linking.openURL(`tel:${selectedPerson?.contactphone}`)
                      }
                    >
                      <Text style={[styles.personDetailValue, darkMode]}>
        ...
        <View style={[styles.PDFContainer, {bottom: 120}]}>
          {pdfPath ? ( // Check if pdf is not null or empty
            ...
            >
              <Text>
                Unable to load the PDF after {maxRetryCount} attempts.
              </Text>
            </View>
          )}
